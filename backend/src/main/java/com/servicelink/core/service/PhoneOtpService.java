...
//        // ✅ Always target the USER's phone number
//        String target = phone.replaceAll("[^\\d]", "");   // strip + from E.164
//
 ...
 * Delivery priority:
 *  1. Sparrow SMS (if credentials present)
 *  2. Twilio WhatsApp API (if credentials present) — sends OTP directly,
 *     fully automated, no manual user action required.
 *  3. wa.me deep-link fallback (last resort)
 */
@Service
public class PhoneOtpService {
    ...
    private SendResult sendViaSparrow(String phone, String otp) {
        try {
            ...
            log.error("Sparrow SMS failed for [{}], falling back: {}", mask(phone), ex.getMessage());
            return isTwilioConfigured()
                    ? sendViaTwilioWhatsApp(phone, otp)
                    : buildWhatsAppFallback(phone, otp);
        }
    }
    ...
    public enum DeliveryMethod { SMS, WHATSAPP_API, WHATSAPP_LINK }

    public record SendResult(DeliveryMethod method, String whatsappLink) {
        public boolean isWhatsApp() { return method == DeliveryMethod.WHATSAPP_LINK; }
        public boolean isAutomated() { return method == DeliveryMethod.SMS || method == DeliveryMethod.WHATSAPP_API; }
    }
