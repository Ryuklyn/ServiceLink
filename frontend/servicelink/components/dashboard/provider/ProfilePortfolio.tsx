...
import { usePortfolio } from "@/lib/hooks/usePortfolio";
import type { PortfolioFormValues, PortfolioProject } from "@/types/portfolio";

const serviceTypes = ["Electrical Wiring", "Electrical Lighting", "Electrical Installation", "Solar Installation"];
const PAGE_SIZE = 6;

/**
 * Full-screen gallery/detail viewer for a single portfolio project.
 * Cycles through all photos + video (if present) with keyboard support.
 */
...
export default function ProfilePortfolio() {
    ...
    const handleDelete = async (e: React.MouseEvent, projectId: number) => {
        e.stopPropagation(); // don't trigger the card's view-modal click
        await removeProject(projectId);
        // If that was the last item on a page beyond the first, step back a page
        if (paginated.length === 1 && page > 1) {
            setPage((p) => p - 1);
                    ...
                    {paginated.map((project) => {
                        ...
                        return (
                            <div
                                ...
                                className="group relative cursor-pointer overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md"
                            >
                                <button
                                ...
                                <div className="relative h-40 w-full bg-gradient-to-br from-[#1e3a8a]/15 to-[#e8683f]/15">
                                    ...
                                    {/* Hover hint that the card opens a full viewer */}
                                    <div className="absolute inset-0 flex items-center justify-center bg-black/0 opacity-0 transition-all duration-200 group-hover:bg-black/20 group-hover:opacity-100">
                                        <div className="flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold text-gray-800 shadow">
