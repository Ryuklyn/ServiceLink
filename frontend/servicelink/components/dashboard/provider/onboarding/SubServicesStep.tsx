 ...
 interface SubServicesStepProps {
     categories: { id: number; name: string }[];   // was: string[]
     onNext: () => void;
     onBack: () => void;
 }
 ...
 export default function SubServicesStep({ categories, onNext, onBack }: SubServicesStepProps) {
     ...
     useEffect(() => {
         categories.forEach((c) => {
             if (!catalogByCategory[c.id]) dispatch(fetchCatalog(c.id));
         });
     }, [categories, catalogByCategory, dispatch]);

     const activeCatalog = activeCategoryId !== undefined ? (catalogByCategory[activeCategoryId] ?? []) : [];
     const totalCatalogItems = categories.reduce(
         (sum, c) => sum + (catalogByCategory[c.id]?.length ?? 0),
         0,
     );
     ...
     const allCatalogsLoaded = categories.every((c) => catalogByCategory[c.id] !== undefined);
     const canProceed =
     ...
     const activeCategoryName = categories.find((c) => c.id === activeCategoryId)?.name ?? "this category";
        ...
        return (
            <div className="px-8 py-8">
                ...
                {categories.length > 1 && (
                    <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                        {categories.map((c) => (
                            ...
                            >
                                {c.name}
                                {catalogByCategory[c.id] && (
                                    <span className="ml-1.5 opacity-70">
                                {Object.values(selections).filter(
                                    (s) => s.isAvailable && catalogByCategory[c.id].some((item) => item.id === s.catalogId),
                                ).length}/{catalogByCategory[c.id].length}
                            </span>
                                )}
                            </button>
                        ))}
                ...
                <div className="mt-5 max-h-[340px] overflow-y-auto pr-1 space-y-2">
                    {loadingCatalog && activeCategoryId !== undefined && !catalogByCategory[activeCategoryId] ? (
                        ...
                        <p className="text-gray-400 text-sm text-center py-10">
                            No catalog items found for {activeCategoryName|| "this category"} yet.
                        </p>
                    ) : (
