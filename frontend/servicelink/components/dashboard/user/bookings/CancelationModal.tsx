      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[11px] font-mono text-gray-400 mt-0.5">
            BK-2024-78542 - CleanNest Services
          </p>
        </div>
      </div>
      <button
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {REASONS.map((r) => (
              <label
                key={r}
                className={`flex items-center gap-3 p-3 rounded-xl border text-xs font-semibold cursor-pointer transition-all ${reason === r ? "border-[#1e3a8a] bg-blue-50/30 text-gray-900" : "border-gray-200 hover:border-gray-300 bg-white text-gray-600"}`}
              >
                <input
                  type="radio"
                  name="freeReason"
...
/* ─────────────────────────────────────────────────────────────
   REDESIGNED OPTION CARDS  (token + fee)  — matches screenshot
───────────────────────────────────────────────────────────── */
function OptionCard({
  selected,
  disabled = false,
  ...
  return (
    <div
      ...
      `}
    >
      {/* Top row: badge left, radio right */}
      ...
      <div className="p-5 space-y-4 overflow-y-auto max-h-[calc(100vh-180px)]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {REASONS.map((r) => (
                  <label
                    key={r}
                    className={`flex items-center gap-3 p-2.5 rounded-xl border text-xs font-semibold cursor-pointer transition-all ${reason === r ? "border-[#1e3a8a] bg-blue-50/30 text-gray-900" : "border-gray-200 hover:border-gray-300 bg-white text-gray-600"}`}
                  >
                    <input
                      type="radio"
                      name="lateReason"
          ...
          <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1">
                      {[0, 1].map((i) => (
                        <div
                          key={i}
                          className={`w-3.5 h-3.5 rounded-full ${i < tokensRemaining ? "bg-[#1e3a8a]" : "bg-gray-300"}`}
                        />
                      ))}
                    </div>
                  </div>
            ...
            {/* Accepted wallets — informational only */}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {[0, 1].map((i) => (
                <div
                  key={i}
                  className={`w-4 h-4 rounded-full transition-colors ${i < tokensRemaining - 1 ? "bg-[#1e3a8a]" : "bg-gray-200"}`}
                />
              ))}
            </div>
          </div>
    ...
    <>
            ...
            {(["esewa", "khalti"] as DigitalWallet[]).map((wallet) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-4 rounded-xl border-2 transition-all text-left ${selected ? (isEsewa ? "border-green-500 bg-green-50" : "border-purple-500 bg-purple-50") : "border-gray-200 bg-white hover:border-gray-300"}`}
                >
                  <div
      ...
      <div className="p-4 bg-gray-50 border-t border-gray-100 flex gap-3">
        ...
        <button
          ...
          onClick={onConfirm}
          className={`flex-1 py-2.5 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 ${!selectedWallet ? "bg-gray-200 text-gray-400 cursor-not-allowed" : selectedWallet === "esewa" ? "bg-green-500 hover:bg-green-600 text-white" : "bg-purple-600 hover:bg-purple-700 text-white"}`}
        >
  ...
  return (
    <>
      <div className="p-6 flex flex-col items-center text-center space-y-5">
        ...
        <div
          className={`w-full border rounded-xl p-4 space-y-3 ${walletBorder}`}
        >
...
export default function CancellationModal({
  ...
  const handleFreeConfirm = () => setStep("success");
  ...
  const handleTokenConfirm = () => setStep("success");
  const handlePaymentNext = () => setStep("confirm-payment");
  const handlePaymentConfirm = () => setStep("success");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-lg rounded-2xl border border-gray-100 shadow-xl overflow-hidden flex flex-col max-h-[95vh]">
        {showHeader && <ModalHeader isLate={isLate} onClose={handleClose} />}
