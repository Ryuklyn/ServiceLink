...
> = {
  ...
  Painting: {
    icon: <Paintbrush2 size={14} strokeWidth={2.2} />,
    color: "#166534",
    bg: "#ecfdf3",
    border: "#a7f3c2",
  },
  ...
  return (
      ...
      >
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          {/* Horizontally Scrollable Tab Pills for Mobile screens */}
            ...
            {tabs.map((tab) => {
              ...
              return (
                  <button
                      ...
                      className="flex items-center gap-1.5 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-200 focus:outline-none whitespace-nowrap flex-shrink-0"
                      style={
                        isActiveTab
                            ? {
                              background:
        ...
        <div className="flex flex-col gap-5 sm:gap-6">
          {visibleCategories.map((cat) => {
            ...
            return (
                ...
                <span
                    className="flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 rounded-full flex-shrink-0"
                  ...
                  <div className="flex flex-col gap-2.5 sm:gap-3">
                    {grouped[cat].map((service) => {
                      ...
                      return (
                          <div
                              key={service.name}
                              className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 rounded-xl p-3 sm:p-4 transition-all duration-200 hover:shadow-sm"
                              style={{
                                border: added
                                    ? "1.5px solid #10b981"
                          ...
                          >
                            ...
                            {/* Right Actions: Flex container stack optimized for inline and wrapped spacing */}
                            <div className="flex items-center xs:justify-end justify-between gap-3 sm:gap-4 flex-shrink-0 pt-2 xs:pt-0 border-t xs:border-t-0 border-gray-100/60 xs:border-none">
                              {/* Price block */}
                              <div className="xs:text-right flex-shrink-0">
                                ...
                                {(service as any).priceNote && (
                                    <p className="text-[11px] sm:text-xs text-gray-400 mt-0.5 max-w-[140px] xs:max-w-none break-words">
                                      {(service as any).priceNote}
                                    </p>
                                )}
                              </div>

                              {/* Category badge (hidden completely on smaller screen bounds) */}
                              <span
                                  className="hidden md:inline-flex text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0"
                                  style={{
                                    background: meta.bg,
                                    color: meta.color,
                              ...
                              <button
                                  ...
                                  className="flex items-center justify-center gap-1 text-xs sm:text-sm font-bold px-3 py-2 sm:px-4 sm:py-2 rounded-lg transition-all duration-150 flex-shrink-0 hover:opacity-90 active:scale-95 w-24 sm:w-auto"
                                  style={
                                    added
                                        ? {
                                          background: "#f1f5f9",
