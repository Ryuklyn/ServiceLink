...
export default function PhoneStep({ onOtpSent }: PhoneStepProps) {
  ...
  const validate = () => {
    ...
    if (!/^(98|97|96)\d{8}$/.test(phone))
      return (
        setError("Enter valid Nepali number (98/97/96 + 8 digits)"),
        false
      );
  ...
  const handleInput = (val: string) => {
    if (loading) return; // block input during request
    const digits = val.replace(/\D/g, "").slice(0, 10);
    setPhone(digits);
  };
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col lg:flex-row bg-white overflow-hidden">
      {/* ───────── LEFT SIDE (same as login page style) ───────── */}
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10 bg-[#f0f4ff] overflow-y-auto">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-blue-100 overflow-hidden">
          ...
          <div className="px-6 py-6 space-y-5">
            {/* Input */}
            <div>
              <label className="text-xs font-semibold text-[#1E3A8A] mb-1.5 uppercase tracking-wide block">
                Mobile Number
              </label>
              ...
              >
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && handleSend()}
                  disabled={loading}
                  className="flex-1 px-4 py-3.5 text-[#1E3A8A] text-base font-mono font-semibold tracking-widest outline-none placeholder:text-gray-300 placeholder:font-normal placeholder:tracking-normal disabled:cursor-not-allowed"
                  maxLength={10}
                  autoFocus
                />

                {/* 🔥 DIGIT COUNTER (kept intact) */}
                <span
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 
                text-white font-bold py-3.5 rounded-xl transition-all duration-200 shadow-md
                ${
                  loading || phone.length !== 10
                    ? "bg-gray-200 text-gray-400 cursor-not-allowed shadow-none"
                    : "bg-[#E8683F] hover:bg-[#d45a32] hover:shadow-lg active:scale-[0.98]"
                }
              `}
            >
              {loading ? (
