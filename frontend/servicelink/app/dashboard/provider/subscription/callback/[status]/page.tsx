...
//     useEffect(() => {
//         const run = async () => {
//             // eSewa: distinct success_url/failure_url, so the path segment
//             // itself tells us the outcome.
//             // Khalti: only ONE return_url for both outcomes — it appends its
//             // own status on top regardless of which path we sent it to, so
//             // the path segment can't be trusted for Khalti; its real ?status=
//             // query param is the source of truth instead.
//             const pathStatus = params?.status;
//
...
//                 const khaltiStatus = searchParams.get("status"); // "Completed" | "Pending" | "Expired" | "User canceled" | etc.
...
//
//     return (
//         <div className="min-h-[100dvh] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
